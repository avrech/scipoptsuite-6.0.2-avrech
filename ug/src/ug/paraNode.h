...
public :
   ...
   ParaNode(
         ...
         estimatedValue(0.0), diffSubproblemInfo(0), diffSubproblem(0), mergingStatus(-1), mergeNodeInfo(0), nodesAreCollected(false)
   {
   }
   ...
   ParaNode( NodeId inNodeId,
         ...
         estimatedValue(inEstimatedValue), diffSubproblem(inDiffSubproblem), mergingStatus(-1), mergeNodeInfo(0), nodesAreCollected(false)
   {
   ...
   bool read(ParaComm *comm, gzstream::igzstream &in, bool onlyBoundChanges); // bool hasMergingStatus);
#endif
   ...
   bool areNodesCollected(){ return nodesAreCollected; }
   void collectsNodes(){ nodesAreCollected = true; }
