...
public :
   ...
   /** se dual bound gain is testing value */
...
public:
   AscendingSelectionHeap(size_t size);
...
public:
   AscendingCollectingModeSolverHeap(size_t size);
...
protected:
   ...
   size_t                               nSolvers;          /**< number of solvers */
   size_t                               nGenerator;        /**< number of generators */
   size_t                               nCollectingModeSolvers; /**< number of collecting mode solvers */
   size_t                               nMaxCollectingModeSolvers; /**< maximum number of solvers that can be in collecting mode */
   size_t                               nLimitCollectingModeSolvers; /**< limit number of solvers that can be in collecting mode */
   unsigned long long                   nNodesSolvedInSolvers; /**< number of nodes solved in current running solvers */
   unsigned long long                   nTotalNodesSolved; /**< number of nodes solved : updated at termination of subtree comp. */
   ...
   int                                  mCollectingNodes;  /**< multiplier for the number of collecting nodes */
   int                                  mMaxCollectingNodes; /**< maximum multiplier for the number of collecting nodes */
...
public:
           ...
           nDualBoundGainTesting(0)
   {
      ...
      if( paraParams->getIntParamValue(MaxNumberOfCollectingModeSolvers) > 0 )
      {
         nMaxCollectingModeSolvers = std::min(
               (size_t)paraParams->getIntParamValue(MaxNumberOfCollectingModeSolvers), nSolvers );
      }
      else if (paraParams->getIntParamValue(MaxNumberOfCollectingModeSolvers) == 0 )
      {
         nMaxCollectingModeSolvers = std::max(nSolvers / 2, (size_t)1);  // at least one solver has to be in collecting mode
      }
      else
      {
   ...
   unsigned long long getNnodesSolvedInSolvers(
         )
   {
   ...
   /** get number of nodes solved in all solvers */
   unsigned long long getTotalNodesSolved(
         )
   {
   ...
   unsigned long long getNnodesInSolvers(
         )
   {
   ...
   size_t getNLimitCollectingModeSolvers(
         )
   {
   ...
   /** get the number of nodes solved by rank of solver */
   long long getNumOfNodesSolved(
         int rank
         )
   {
   ...
   int getInactiveSolverRank(
         )
   {
      std::map<int, ParaSolverPoolElementPtr>::iterator p;
   ...
   /** get multiplier of collecting nodes */
   int getMCollectingNodes(){ return mCollectingNodes; }
   int getMMaxCollectingNodes(){ return mMaxCollectingNodes; }

   /** the following functions are to omit rebooting collecting mode process */
   double getSwichOutTime(){ return switchOutTime; }
   void   setSwichOutTime(double time){ switchOutTime = time; }

   bool isDualBounGainTesting(int rank){ return pool[SOLVER_POOL_INDEX(rank)]->isDualBoundGainTesting(); }

   // bool beforeInitialCollectStage(){ return beforeInitialCollect; }
};
...
public:
   ...
   ParaSolverPoolForMinimization(
         ...
         : ParaSolverPool(inMp, inBgap, inMBgap, inOriginRank, inParaComm, inParaParams, inParaTimer)
   {
      ...
      if( paraParams->getIntParamValue(SolverOrderInCollectingMode) == 0) // ordered by dual bound value
      {
         collectingModeSolverHeap = new AscendingCollectingModeSolverHeap(
               std::min( nMaxCollectingModeSolvers * (size_t) 2, nSolvers ) );       // can be more than nMaxCollectingModeSolvers
      }
   }
