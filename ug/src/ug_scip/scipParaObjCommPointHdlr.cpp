      ...
      )
{
                  ...
                  && SCIPgetStage(scip) != SCIP_STAGE_INITSOLVE )
   {
         ...
         )
      {
         ...
         {
            if( SCIPfeasCeil(scip, scipParaSolver->getCurrentNode()->getDualBoundValue())
   ...
   {
      assert( scipParaSolver->getCurrentNode()->isRootNode() );
   ...
   if( nNodes == 2 )
   {
      ...
      {
         if( ( SCIPgetDualbound(scip) - scipParaSolver->getCurrentNode()->getDualBoundValue() )
   ...
   {
      bestDualBoundValue = std::max(SCIPgetDualbound(scip), scipParaSolver->getCurrentNode()->getDualBoundValue());
   }
...
//   }
         ...
         || scipParaSolver->getParaParamSet()->getBoolParamValue(UG::Deterministic) )  // always notify
   {
      if( bestDualBoundValue >= -1e+10 )  // only after some dual bound value is computed, notify its status
      {
         ...
         if( scipParaSolver->getParaParamSet()->getBoolParamValue(UG::GenerateReducedCheckpointFiles)  &&
               nNodes > 2 )
         {
            scipParaSolver->getCurrentNode()->setMergingStatus(3);  // cannot be merged: this is a dummy status
            SCIP_CALL_ABORT( SCIPinterruptSolve(scip) );
            return SCIP_OKAY;
         }
         if( scipParaSolver->getParaParamSet()->getBoolParamValue(UG::Deterministic)
   ...
   if( scipParaSolver->getNStopSolvingMode() > 0 &&
         ...
         / std::max(std::abs(scipParaSolver->getLcBestDualBoundValue()), 1.0) ) > scipParaSolver->getBoundGapForStopSolving() )
   {
   ...
   if( scipParaSolver->getCurrentNode()->isRootNode() &&
