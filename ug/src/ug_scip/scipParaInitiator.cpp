      ...
      )
{
   ...
   if ( std::string(paraParams->getStringParamValue(UG::RacingParamsDirPath)) != std::string("") )
   {
      ...
      for( int n = 1; n < scipParaComm->getSize() ; n++ )
      {
         ...
         SCIP_CALL_ABORT( SCIPresetParams(scip) );
         SCIP_CALL_ABORT( SCIPreadParams(scip, oss.str().c_str()) );
      ...
      )
{
   ...
   if( ( instance->getOrigObjSense() == SCIP_OBJSENSE_MINIMIZE && (convertToExternalValue(solution->getObjectiveFuntionValue()) - updateIncumbent->getObjective() ) > MINEPSILON ) ||
         ( instance->getOrigObjSense() == SCIP_OBJSENSE_MAXIMIZE && ( updateIncumbent->getObjective() - convertToExternalValue(solution->getObjectiveFuntionValue()) ) > MINEPSILON ) )
   {
      ...
      )
{
   ...
   UGS::UgsUpdateIncumbent *uui = new UGS::UgsUpdateIncumbent(ugsComm->getMySolverId(), seqNumber, convertToExternalValue(solution->getObjectiveFuntionValue()) );
