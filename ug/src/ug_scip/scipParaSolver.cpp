   ...
   else
   {
      ...
      else
      {
         ...
         switch( nPresolving )
         {
            ...
            {
#ifdef _COMM_PTH
               if( paraParams->getBoolParamValue(UG::CustomizedToSharedMemory ) )
               {
                  SCIP_CALL_ABORT( SCIPsetPresolving(scip, SCIP_PARAMSETTING_DEFAULT, TRUE) );
               }
               else
      ...
      )
{
   ...
   double dualBoundValue = currentNode->getDualBoundValue();
   ...
   {
      if( ( addingConsParam == 1 && !SCIPisGT(scip, currentNode->getDualBoundValue(), currentNode->getInitialDualBoundValue()) )
      ...
      )
{
   ...
   {
      double timeRemains =  paraParams->getRealParamValue(UG::TimeLimit) - paraTimer->getElapsedTime() + 60.0;  // 60.0: timming issue
   ...
   else
   {
      ...
      if( SCIPdebugSolIsEnabled(scip) && 
            currentNode->getMergingStatus() != 3 &&
   ...
   else
   {
      ...
      {
         throw "SCIP terminated with SCIP_STATUS_MEMLIMIT";
      }
      ...
      {
         if( SCIPfeasCeil(scip, getCurrentNode()->getDualBoundValue()) >= getGlobalBestIncumbentValue() )
         {
            solverDualBound = SCIPfeasCeil(scip, getCurrentNode()->getDualBoundValue());
         }
         else
         {
            solverDualBound = std::max(getCurrentNode()->getDualBoundValue(), SCIPgetDualbound(scip));
         }
      }
      else
      {
      ...
      copyIncreasedVariables(false)
{
   ...
   if( paraParams->getRealParamValue(UG::TimeLimit) > 0.0 )
   {
      double timeRemains =  paraParams->getRealParamValue(UG::TimeLimit) - paraTimer->getElapsedTime() + 3.0;  // 3.0: timming issue
      SCIP_CALL_ABORT( SCIPsetRealParam(scip,"limits/time", timeRemains) );
   }
   /** set user plugins */
   ::setUserPlugins(this);
      ...
      )
{
   ...
   assert( nOrgVarsInSolvers == paraInstance->getVarIndexRange() );
...
void
ScipParaSolver::reinitializeInstance(
      )
{
   /*************************************
      ...
      )
{
   ...
   if( strcmp(bakFileName,"-") != 0 )
   {
      ...
      SCIP_CALL_ABORT( SCIPsetStringParam(scip,"visual/baknodeprefix", ((currentNode->getNodeId()).toString()+":").c_str() ) );
      SCIP_CALL_ABORT( SCIPsetStringParam(scip,"visual/bakrootinfo", (currentNode->getGeneratorNodeId()).toString().c_str() ) );
      ...
      )
{
   ...
   /* constraint handlers */
   SCIP_CALL_ABORT( SCIPsetBoolParam(scip, "constraints/abspower/dualpresolve", abspowerDualpresolve) );
   SCIP_CALL_ABORT( SCIPsetBoolParam(scip, "constraints/and/dualpresolving", andDualpresolving) );
   SCIP_CALL_ABORT( SCIPsetBoolParam(scip, "constraints/cumulative/dualpresolve", cumulativeDualpresolve) );
   SCIP_CALL_ABORT( SCIPsetBoolParam(scip, "constraints/knapsack/dualpresolving", knapsackDualpresolving) );
   SCIP_CALL_ABORT( SCIPsetBoolParam(scip, "constraints/linear/dualpresolving", linearDualpresolving) );
