      ...
      )
{
   ...
   if( commTh->getRank() == root )
   {
      for( int i = 0; i < commTh->getSize(); i++ )
      {
         if( i != root )
         {
            ...
            sent = createDatatype();
            PARA_COMM_CALL(
               commTh->uTypeSend((void *)sent, ParaSolverDiffParamType, i, TagSolverDiffParamSet)
            );
         }
      }
   }
   else
   {
      ...
      PARA_COMM_CALL(
         commTh->uTypeReceive((void **)&received, ParaSolverDiffParamType, root, TagSolverDiffParamSet)
      );
      setValues(received);
      delete received;
   }
      ...
      )
{
   ...
   PARA_COMM_CALL(
      commTh->uTypeSend((void *)createDatatype(), ParaSolverDiffParamType, dest, TagSolverDiffParamSet)
   );
...
ScipDiffParamSetTh::receive(
       ...
       )
 {
   ...
   ScipDiffParamSetTh *received;
   PARA_COMM_CALL(
      commTh->uTypeReceive((void **)&received, ParaSolverDiffParamType, source, TagSolverDiffParamSet)
   );
   setValues(received);
   delete received;
